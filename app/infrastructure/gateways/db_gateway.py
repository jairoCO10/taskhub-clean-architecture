...
class UserGateway:
    ...
    async def get_user(self, user_id: int) -> User:
        ...
        if orm_user:
            return User(id=orm_user.id, name=orm_user.name, email=orm_user.email, tasks=[])
        return None

    async def get_users(self, skip: int = 0, limit: int = 10) -> List[User]:
        orm_users = await self.repository.get_users(skip, limit)
        return [User(id=user.id, name=user.name, email=user.email, tasks=[]) for user in orm_users]
